...
public sealed class HeadsetSystem : SharedHeadsetSystem
{
    ...
    private Dictionary<int, string> GetFormattedStations(EntityUid player)
    {
        ...
        if (!_mind.TryGetLegalID(player, out var legalId))
            return formattedStations;

        foreach (var station in _station.GetStations())
        {
            if (!TryComp<CrewRecordsComponent>(station, out var crewRecords) || !crewRecords.TryGetRecord(legalId, out _))
                continue;
        ...
        if (TryComp<StationDataComponent>(faction, out var sD) && sD != null)
        {
            if (sD.RadioData.ContainsKey(channel.ID))
            {
                if (sD.RadioData.TryGetValue(channel.ID, out var data) && data != null)
                {
                    ...
                    {
                        if (crewRecords.TryGetRecord(legalId, out var crewRecord) && crewRecord != null)
                        {
    ...
    private void OnSpeak(EntityUid uid, WearingHeadsetComponent component, EntitySpokeEvent args)
    {
        if (args.Channel != null)
        {
            if (TryComp<HeadsetComponent>(component.Headset, out var headsetComp) && headsetComp != null)
            {
                ...
                if (headsetComp.TransmitTo.Count <= 0)
                    return;

                if (args.EncryptionID is { } targetedEncryptionId && targetedEncryptionId > 0)
                {
                    if (!headsetComp.TransmitTo.Contains(targetedEncryptionId))
                        return;

                    var targetedFaction = _station.GetStationByID(targetedEncryptionId);
                    if (targetedFaction != null && HasChannelAccess(args.Source, targetedFaction.Value, args.Channel))
                    {
                ...
                var sent = false;
                foreach (var transmitStation in headsetComp.TransmitTo)
                {
                    var faction = _station.GetStationByID(transmitStation);
                    if (faction != null)
                    {
                        if (HasChannelAccess(args.Source, faction.Value, args.Channel))
                        {
                            _radio.SendRadioMessage(uid, args.Message, args.Channel, component.Headset, encryptionID: transmitStation);
                            sent = true;
                        }
                    }
                }
